...
exports.meshToGlb = meshToGlb;
function meshToGlb(mesh, name = 'Mesh') {
    ...
    if (normBuf)
        writeOffset = copyAligned(binBuf, normBuf, writeOffset);
    if (uvBuf)
        writeOffset = copyAligned(binBuf, uvBuf, writeOffset);
    ...
    off += 4; // version
    glb.writeUInt32LE(totalSize, off);
    off += 4; // total length
    // JSON chunk
...
function uint32Array(arr) {
    const buf = Buffer.alloc(arr.length * 4);
    for (let i = 0; i < arr.length; i++)
        buf.writeUInt32LE(arr[i], i * 4);
    return buf;
}
function alignTo4(n) {
    return (n + 3) & ~3;
}
function copyAligned(dest, src, offset) {
...
//# sourceMappingURL=mesh-to-glb.js.map